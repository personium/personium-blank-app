...
gulp.task('copy_statics', () => {
  const tasks = config.personium.DIRECTORY_MAPPING.map(mapping => {
    ...
    return gulp.src(mapping.filePattern, { base: mapping.srcDir })
...
async function deployCollection(client, srcFolder, dstName) {
  ...
  try {
    ...
    if (e.response.status === 405) {
      console.log(`the collection ${collectionPath} is already exists (perhaps)`)
    } else {
      console.log(e);
      throw e;
    }
...
async function deployEngine(client, engineFolderPath, engineName) {
  ...
  const results = await Promise.all(files.map(fileInfo => {
    console.log(`engine upload: ${fileInfo.filepath}`);
    return client.putFileToEngine(enginePath, fileInfo.filepath, fileInfo.filename);
  }));
...
gulp.task('deploy', () => {
  ...
  return (async () => {
    const tokens = await PersoniumAuthClient.getTokenROPC(cellURL, username, password);
